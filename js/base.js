...
;(function(){
    ...
    stpo.pxValue = function(emValue){
        return emValue * (parseFloat(w.getComputedStyle(d.getElementsByTagName('body')[0], null).getPropertyValue('font-size')));
    };
    ...
    stpo.toggle = function(){
        ...
        let mobileBreakpoints = "(breakpoint === 'large') || (breakpoint === 'medium') || (breakpoint === 'small') || (breakpoint === 'xsmall') || (breakpoint === 'xxsmall')",
        ...
        const
            ...
            toggleCore = function(target, opener, options, e){
                ...
                if ((target.getAttribute('aria-hidden') === 'true') && (e.key !== 'Escape')){
                    ...
                    if (options.isCollection === "true"){
                        ...
                        if (parent !== null){
                            parent.querySelectorAll('[data-collection]').forEach(function(elm){
                                if ((elm !== opener)&&(elm.getAttribute('aria-expanded') === 'true')){
                                    elm.click();
                                }
                            });
                        }
                    ...
                    Array.prototype.forEach.call(d.querySelectorAll('.plyr-media'), function(myPlayer) {
        ...
        // FOR EACH
        // ------------------------
        Array.prototype.forEach.call(d.querySelectorAll('.toggle'), function(opener){

            // INIT
            // ------------------------
            const
                // find target (or take opener next sibling)
                target = d.querySelector('#' + opener.getAttribute('aria-controls')) || opener.nextElementSibling,
                ...
                options = {
                    closeDuration:  (overlay !== null) ? (parseFloat(getComputedStyle(overlay).transitionDuration) * 1000 || 0) : 0,
                    isTrap:         opener.getAttribute('data-trap') || 'false',
                    isFreeze:       opener.getAttribute('data-freeze') || 'false',
                    isClickOutside: opener.getAttribute('data-click-outside') || 'false',
            ...
            opener.addEventListener('click', (e) => toggleCore(target, opener, options, e));
            ...
            Array.prototype.forEach.call(closers, function(el) {
                ...
                if (!el._toggleCore) {
                    el._toggleCore = true;
                    el.addEventListener('click', (e) => toggleCore(target, opener, options, e));
                }
            });

            // if allowed, add listener on overlay click
            if (options.isClickOutside === 'true'){
                overlay.addEventListener('click', (e) => toggleCore(target, opener, options, e));
            }
            ...
            if (options.isBurger === "true"){
                ...
                const burgerWatch = function(e){
                    ...
                    if (breakpoint !== lastBreakpoint){
                        ...
                        target.querySelectorAll('.toggle[data-collection="true"][aria-expanded="true"]').forEach(function(collection){
                            collection.click();
                        });
